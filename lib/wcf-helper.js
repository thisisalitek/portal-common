...
class WcfHelper {
	constructor(url, username, password, namespace) {
		...
		// })
		console.log(`WcfHelper constructor calisti:`, url, username, password)
	...
	createClient(cb) {
		...
		soap.createClient(this.url, wsdlOptions, (err, client) => {
			if(!err) {
				var options = {
					hasNonce: true,
					actor: 'actor'
				}
				var wsSecurity = new soap.WSSecurity(this.username, this.password, options)
	...
	send(funcName, query, callback) {
		...
		this.createClient((err, client) => {
			if(err) {
				console.log(`send createClient err:`, err)
				return callback(err)
			}
			// var message = this.queryContext(funcName, query)
			// tempLog('sendMessage.xml', message)
			// client.post(message, `http://tempuri.org/${this.namespace}/${funcName}`, (response, ctx) => {
			console.log(`query:`,query)
			client[funcName](query, (response, ctx) => {
