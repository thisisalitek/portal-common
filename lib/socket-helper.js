exports.start= function (server, passportFunc) {
	if(!server)
		return
	let socketCorsDomainList=['https://portal.tr216.com','http://portal.tr216.com','http://tr216.com','http://www.tr216.com','http://portal.ganygo.com','http://ganygo.com','http://www.ganygo.com']
...
exports.sendTotalUnread=function(socket){
	let filter={memberId:socket.memberId,dbId:socket.dbId,isRead:false}
	
	db.notifications.find(filter).sort({_id:-1}).exec((err,docs)=>{
		if(!err){
			let totalUnread=docs.length
			let dizi=[]
